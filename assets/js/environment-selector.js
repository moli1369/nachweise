(() => {
  const load = (url, id) => {
    ...
    const s=document.createElement('script'); s.id=id; s.src=url+(url.includes('?')?'&':'?')+'v=3'; s.defer=true; document.body.appendChild(s);
  };
  const loadCss = (href, id) => {
    if (document.getElementById(id)) return;
    const l=document.createElement('link'); l.id=id; l.rel='stylesheet'; l.href=href+'?v=3'; document.head.appendChild(l);
  };
  ...
  style.textContent=`
    ...
    #environment-selector{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;overflow:auto;padding:22px;background:radial-gradient(circle at 50% 25%,rgba(66,199,255,.13),transparent 33%),linear-gradient(180deg,#030a12,#06111c);color:#edf7ff;font-family:var(--sans,system-ui,sans-serif)}
  ...
  const enter=env=>{sessionStorage.setItem(storageKey,env);root.style.opacity='0';root.style.transition='opacity .22s ease';setTimeout(()=>{document.body.classList.remove('environment-lock');root.remove();window.dispatchEvent(new CustomEvent('portfolio:environment',{detail:{environment:env}}))},230)};
